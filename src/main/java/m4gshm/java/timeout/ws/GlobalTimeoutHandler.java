package m4gshm.java.timeout.ws;
...
import m4gshm.java.timeout.http.HttpHeaders;
import m4gshm.java.timeout.http.HttpStatuses;
...
public class GlobalTimeoutHandler implements SOAPHandler<SOAPMessageContext> {
    ...
    public GlobalTimeoutHandler(TimeLimitExecutor executor) {
        this(executor, HttpHeaders.DEADLINE_HEADER, HttpHeaders.DEADLINE_EXCEED_HEADER, HttpHeaders.DEADLINE_CHECK_TIME_HEADER, HttpStatuses.GATEWAY_TIMEOUT,
    ...
    private void handleResponse(SOAPMessageContext context) {
        ...
        if (rawStatus instanceof Integer && (Integer) deadlineExceedStatus == rawStatus) {
            ...
            val exception = HttpDeadlineHelper.newDeadlineExceedExceptionFromHeaders(headers,
