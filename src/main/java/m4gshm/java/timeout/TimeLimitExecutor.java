...
import static m4gshm.java.timeout.DeadlineExceededException.throwDefaultException;
...
public interface TimeLimitExecutor {
    ...
    default <T> T call(Instant deadline, Function<Context<T>, T> contextConsumer) {
        return call(deadline, contextConsumer, DeadlineExceededException.throwDefaultException());
    }
    ...
    default <T> T call(Function<Context<T>, T> contextConsumer) {
        return call(contextConsumer, DeadlineExceededException.throwDefaultException());
    }
    ...
    default void run(Consumer<Context<Void>> contextConsumer) {
        run(contextConsumer, DeadlineExceededException.throwDefaultException);
    }
    ...
    default void run(Instant deadline, Consumer<Context<Void>> contextConsumer) {
        run(deadline, contextConsumer, DeadlineExceededException.throwDefaultException);
    }
    ...
    default void run(Instant deadline, Runnable runnable) {
        run(deadline, runnable, DeadlineExceededException.throwDefaultException);
    }
