...
import timeout.TimeLimitExecutor.Context;
import timeout.TimeLimitExecutor.ContextImpl;
import timeout.TimeLimitExecutor.DeadlineExceedFunction;

import java.time.Instant;

import static timeout.DeadlineExceededException.newDeadlineExceededException;
import static timeout.TimeLimitExecutorImpl.*;
...
public class TimeLimitedMono<T> extends Mono<T> {
    ...
    class TimeLimitedSubscriber<T> implements CoreSubscriber<T>, Scannable {
        ...
        final DeadlineExceedFunction<T> CALL_ON_ERROR = (c, d) -> {
            onError(newDeadlineExceededException(c, d));
            return null;
        };
